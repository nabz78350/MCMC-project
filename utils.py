...
def kendall_tau_distance(df1, df2):
    ...
    for i in range(len(df1.columns)):
        for j in range(i + 1, len(df1.columns)):
            tau_dist += np.abs(
                np.mean(np.sign(df1[df1.columns[i]] - df1[df1.columns[j]]) *
...
def cholesky_synthetic_data(num_samples, correlation_matrix):
    ...
    synthetic_data = np.random.normal(0, 1, size=(num_samples, len(correlation_matrix))) @ cholesky_matrix.T
    return  synthetic_data 

def filter_positive_extremes(synthetic_data, significance_level=0.95):
   
    theoretical_percentile = np.percentile(np.random.normal(0, 1, size=(100000, synthetic_data.shape[1])), significance_level * 100, axis=0)


    positive_filter = np.all((synthetic_data < theoretical_percentile) & (synthetic_data > 0), axis=1)
...
def adjust_for_correlation(synthetic_data, original_data):
    ...
    adjusted_synthetic_data = synthetic_cholesky @ np.linalg.inv(target_cholesky) @ synthetic_data.T
    adjusted_synthetic_data = adjusted_synthetic_data.T 
    return adjusted_synthetic_data[np.all(adjusted_synthetic_data>0,axis=1)] ## justin case
...
def cholesky(target_sample_size,cov_matrix):
    ...
    while current_sample_size < target_sample_size:
        ...
        synthetic_batch = filter_positive_extremes(synthetic_batch, significance_level=0.99)
        ...
        if current_sample_size + current_batch_size <= target_sample_size:
            ...
            synthetic_data = np.vstack([synthetic_data, synthetic_batch[:target_sample_size - current_sample_size]])
...
def compare(synthetic_data,df_train):
    ...
    sns.heatmap(df_train.corr(), 
                ...
                cmap=sns.diverging_palette(h_neg=20,
                                            h_pos=220), center=0).set(title='Original data - Log returns correlation')
