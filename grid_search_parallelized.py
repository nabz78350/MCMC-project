...
class Generator:
    ...
    def build_model(self):
        ...
        for i in range(1, self.num_layers):
            ...
            model.add(Dense(self.neurons_per_layer[i], activation=self.hidden_activation[i]))
        model.add(BatchNormalization())
...
class GAN:
    ...
    def generate_noise(self,n_samples):
        ...
        noise = np.random.multivariate_normal(mean=np.zeros(self.opt.latent_dim),
    ...
    def train_gan(self,df_train):
        ...
        for epoch in range(self.opt.n_epochs):
            for batch in range(bat_per_epo):
                ...
                real_samples = df_train.values[np.random.choice(df_train.shape[0], self.opt.batch_size, replace=False)]
                fake_samples,labels_fake = self.generate_fake_normal(self.opt.batch_size)

                labels_real = np.ones((self.opt.batch_size, 1))
                labels_fake = np.zeros((self.opt.batch_size, 1))
    
                real_loss = self.discriminator.model.train_on_batch(real_samples, labels_real)
                fake_loss = self.discriminator.model.train_on_batch(fake_samples, labels_fake)
            ...
            print( "[Epoch %d/%d] [D loss Fake: %f] [D loss real:%f] [G loss: %f]" % (epoch, self.opt.n_epochs, fake_loss,real_loss, g_loss)
                )
...
def generate_combination_name(g_config, d_config, latent_dim):
    ...
    g_desc = create_layer_description(g_config['num_layers'], g_config['neurons_per_layer'], g_config['hidden_activation'], g_config['output_activation'])
...
def evaluate_model(g_config, d_config, latent_dim, df_train, results_dir):
    ...
    model_params = {'generator': g_config, 'discriminator': d_config, 'latent_dim': latent_dim}
...
def grid_search(grid_params, df_train, results_dir='results'):
    ...
    Parallel(n_jobs=3)(delayed(evaluate_model)(
        g_config, d_config, latent_dim, df_train, results_dir
...
if __name__ == '__main__':
    ...
    discriminator_combinations = [
        ...
        for neurons in [8,16,32,64]
        for activation in ['relu','leaky_relu','softplus',]
        for output_activation in ['sigmoid','softplus',]
    ]
    ...
    df_train = open_data()  # Make sure this function is defined to load your training data
